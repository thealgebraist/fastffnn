...
int main() {
    cout << "BN + Diversity Curriculum Training (600s)..." << endl;
    ...
    // Greedy Diversity Sampling for Initial Set
    vector<int> current_subset;
    current_subset.push_back(indices[0]);
    for(int i=1; i<INITIAL_IMAGES; ++i) {
        // Sample 100 candidates and pick the most distant from current mean
        int best_cand = -1;
        float max_dist = -1;
        for(int j=0; j<20; ++j) {
    ...
    while (true) {
        ...
        // Batch selection: pick most diverse set of 64 from random pool
        ...
        for (int b = 0; b < BATCH_SIZE; ++b) {
            for(int i=0; i<net.H; ++i) var[i] += (hs[b][i] - mu[i]) * (hs[b][i] - mu[i]);
        }
        ...
        for (int b = 0; b < BATCH_SIZE; ++b) {
            ...
            for(int i=0; i<NUM_CLASSES; ++i) {
                db2[i] += dLogits[i];
                for(int j=0; j<net.H; ++j) dW2[i * MAX_NEURONS + j] += dLogits[i] * hs[b][j];
            }
            ...
            for(int i=0; i<net.H; ++i) {
                if(hs[b][i] > 0) {
                    ...
                    dG[i] += dReLU * hs_norm[b][i];
                    dB[i] += dReLU;
                    
                    // Simplified BN backprop
                    float dNorm = dReLU * net.bn_gamma[i];
                    db1[i] += dNorm / var[i];
                    for(int j=0; j<INPUT_DIM; ++j) dW1[i * INPUT_DIM + j] += (dNorm / var[i]) * all_images[batch_indices[b] * 3072 + j];
                }
            }
        }
        ...
        if (avg_loss < 0.05) {
            if (net.t % 50 == 0) {
                cout << "Batch Reached target accuracy. Adding 128 more images." << endl;
