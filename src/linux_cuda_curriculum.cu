...
__global__ void bn_lrelu_forward_kernel(float* hs, float* hs_norm, const float* b1, const float* gamma, const float* beta, float* mu, float* var, int H, int B) {
    ...
    if (i < H) {
        ...
        for(int b=0; b<B; ++b) {
            ...
            hs[b * H + i] = act > 0 ? act : 0.1f * act; // Leaky ReLU
        }
    }
}
...
__global__ void bn_backprop_kernel(const float* dL_dhs, const float* hs_pre, const float* h_norm, const float* gamma, const float* beta, const float* var, float* db1, float* dG, float* dB, float* dh_scaled, int H, int B) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < H) {
        float sum_dl_dy = 0, sum_dl_dy_xhat = 0;
        for(int b=0; b<B; ++b) {
            // Re-calculate the pre-activation to get Leaky ReLU derivative
...
int main() {
    cout << "Corrected Fused Kernel CUDA Training..." << endl;
    download_cifar10();
    ...
    mt19937 gen(42);
    float s1 = sqrtf(2.0f / (INPUT_DIM + H)), s2 = sqrtf(2.0f / (H + NUM_CLASSES)); 
    ...
    CudaVector hs(BATCH_SIZE * MAX_NEURONS), hs_norm(BATCH_SIZE * MAX_NEURONS), logits(BATCH_SIZE * NUM_CLASSES), dLogits(BATCH_SIZE * NUM_CLASSES), dL_dhs(BATCH_SIZE * MAX_NEURONS);
    ...
    while (chrono::duration_cast<chrono::seconds>(chrono::high_resolution_clock::now() - start_time).count() < TRAIN_LIMIT_S) {
        *loss_gpu = 0; *correct_gpu = 0;
        CHECK_CUDA(cudaMemset(dW1.data(), 0, dW1.size()*4)); CHECK_CUDA(cudaMemset(db1.data(), 0, MAX_NEURONS*4));
        CHECK_CUDA(cudaMemset(dW2.data(), 0, dW2.size()*4)); CHECK_CUDA(cudaMemset(db2.data(), 0, NUM_CLASSES*4));
        CHECK_CUDA(cudaMemset(dG.data(), 0, MAX_NEURONS*4));  CHECK_CUDA(cudaMemset(dB.data(), 0, MAX_NEURONS*4));
        ...
        // hs = W1 * batch_imgs. W1 is Hx3072, batch_imgs is 3072xBATCH_SIZE
        ...
        // dW2 = dLogits * hs^T. dLogits is CxB, hs is HxB
        CHECK_CUBLAS(cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, NUM_CLASSES, H, BATCH_SIZE, &alpha, dLogits.data(), NUM_CLASSES, hs.data(), H, &beta, dW2.data(), NUM_CLASSES));
        // dL_dhs = W2^T * dLogits. W2 is CxH, dLogits is CxB
        CHECK_CUBLAS(cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, H, BATCH_SIZE, NUM_CLASSES, &alpha, W2.data(), NUM_CLASSES, dLogits.data(), NUM_CLASSES, &beta, dL_dhs.data(), H));
        
        bn_backprop_kernel<<<(H+255)/256, 256>>>(dL_dhs.data(), hs.data(), hs_norm.data(), bn_gamma.data(), bn_beta.data(), var.data(), db1.data(), dG.data(), dB.data(), dh_scaled.data(), H, BATCH_SIZE);
        // dW1 = dh_scaled * batch_imgs^T. dh_scaled is HxB, batch_imgs is 3072xB
        CHECK_CUBLAS(cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, H, INPUT_DIM, BATCH_SIZE, &alpha, dh_scaled.data(), H, batch_imgs.data(), INPUT_DIM, &beta, dW1.data(), MAX_NEURONS));
        
        float avg_loss = *loss_gpu / BATCH_SIZE; t++;
        ...
        if (chrono::duration_cast<chrono::seconds>(chrono::high_resolution_clock::now() - last_print_time).count() >= 1) {
            cout << "[Time: " << (int)chrono::duration_cast<chrono::seconds>(chrono::high_resolution_clock::now() - start_time).count() << "s] Err: " << (1.0f - (*correct_gpu / (float)BATCH_SIZE)) * 100.0f << "% | Loss: " << avg_loss << " | Subset: " << current_subset.size() << " | Neurons: " << H << endl;
            last_print_time = chrono::high_resolution_clock::now();
        }
        if (abs(avg_loss - last_loss) < 1e-4) stagnate_count++; else stagnate_count = 0;
        last_loss = avg_loss;
        if (stagnate_count >= 4) { H += 2; stagnate_count = 0; cout << "[Stagnation] Expanding to " << H << " neurons." << endl; }
